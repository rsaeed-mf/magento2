...
$mfLibFile = __DIR__ . '/Library/MyfatoorahApiV2.php';

if (!file_exists($mfLibFile) || (time() - filemtime($mfLibFile) > 86400)) {

    $mfCurl = curl_init('http://localhost/library/2.0.0/MyfatoorahLibrary.txt');
    ...
    if ($mfHttpCode == 200) {
        ...
        $mfNamespace = '<?php namespace MyFatoorah\Gateway\Library; ';
        $mfUse1      = 'use MyFatoorah\Gateway\Library\MyfatoorahApiV2; ';
        $mfUse2      = 'use Exception; ';
        $mfClass     = 'class ';

        $mfSplitFile = explode('class', $mfResponse);

        file_put_contents(__DIR__ . '/Library/MyfatoorahApiV2.php', $mfNamespace . $mfUse2 . $mfClass . $mfSplitFile[1]);
        file_put_contents(__DIR__ . '/Library/PaymentMyfatoorahApiV2.php', $mfNamespace . $mfUse1 . $mfUse2 . $mfClass . $mfSplitFile[2]);
        file_put_contents(__DIR__ . '/Library/ShippingMyfatoorahApiV2.php', $mfNamespace . $mfUse1 . $mfClass . $mfSplitFile[3]);
    }
}
